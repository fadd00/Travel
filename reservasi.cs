...
namespace Travel
{
    public partial class PreviewForm : Form
    {
                                ...
                                using (SqlCommand cmd = new SqlCommand(insertQuery, connection, transaction)) // Diubah ke SqlCommand
                                {
                                    ...
                                    foreach (DataRow row in dataToImport.Rows)
                                    {
                                        try
                                        {
                                            ...
                                            {
                                                object waktuValue = row["waktu"];
                                                TimeSpan parsedTime = TimeSpan.Zero; // Default value
                                                bool conversionSuccessful = false;

                                                if (waktuValue is DateTime dtObject) // Case 1: Already a DateTime object
                                                {
                                                    parsedTime = dtObject.TimeOfDay;
                                                    conversionSuccessful = true;
                                                }
                                                else
                                                {
                                                    string waktuString = waktuValue.ToString();
                                                    // Case 2: Try to parse string as DateTime (e.g., "12/30/1899 14:30:00")
                                                    if (DateTime.TryParse(waktuString, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out DateTime dtFromString))
                                                    {
                                                        parsedTime = dtFromString.TimeOfDay;
                                                        conversionSuccessful = true;
                                                    }
                                                    // Case 3: Try to parse string as TimeSpan (e.g., "14:30")
                                                    else if (TimeSpan.TryParse(waktuString, out parsedTime))
                                                    {
                                                        conversionSuccessful = true;
                                                    }
                                                }
                                                ...
                                                {
                                                    errorCount++;
                                                    errorMessages.AppendLine($"Baris {successCount + errorCount} (Waktu): Format waktu '{waktuValue}' tidak valid. Tidak dapat dikonversi ke format waktu yang benar.");
                                                    continue; // Skip this row
                                                }
                                            }
        ...
        private string ValidateRow(DataRow row)
        {
            ...
            if (!dataToImport.Columns.Contains("waktu") || row["waktu"] == DBNull.Value || string.IsNullOrWhiteSpace(row["waktu"]?.ToString()))
            ...
            else
            {
                ...
                if (waktuValue is DateTime) // Already a DateTime object
                {
                    isValidTime = true;
                }
                else
                {
                    string waktuString = waktuValue.ToString();
                    // Try to parse string as DateTime (e.g., "12/30/1899 14:30:00")
                    if (DateTime.TryParse(waktuString, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out _))
                    {
                        isValidTime = true;
                    }
                    // Try to parse string as TimeSpan (e.g., "14:30")
                    else if (TimeSpan.TryParse(waktuString, out _))
                    {
                        isValidTime = true;
                    }
                }
                ...
                if (!isValidTime)
                {
                    return $"Format waktu '{waktuValue}' pada kolom 'waktu' tidak valid.";
                }
            }
